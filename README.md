...
- **AIボタン**: 録音内容に対するAI応答を生成（Groq使用時はGroq、それ以外の場合はGemini APIで応答生成）
  - **システムプロンプト編集**: AI応答のシステムプロンプトを設定画面からカスタマイズ可能
  - **Google検索グラウンディング**: Gemini API使用時にGoogle検索結果を活用した応答生成（オプション）
...
- Androidアプリの設定画面で「Cloud Speech-to-Text API Key」にAPIキーを入力
...
- **システムプロンプト編集**: Androidアプリの設定画面からAI応答のシステムプロンプトをカスタマイズできます。録音内容に対してAIがどのような応答を生成するかを細かく制御できます。

- **Google検索グラウンディング**: Gemini API使用時にGoogle検索結果を活用した応答生成が可能です（オプション）。最新情報を含めた応答が生成できます。設定画面から有効/無効を切り替えられます。
