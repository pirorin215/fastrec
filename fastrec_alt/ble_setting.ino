...
// BLE File Transfer Protocol Constants
#define BLE_PACKET_SIZE 512           // Total packet size for file transfer
#define BLE_PAYLOAD_SIZE 508          // File data payload per packet
#define BLE_HEADER_SIZE 4             // Chunk counter header (32-bit LE)
#define BLE_MTU_SIZE 517              // Maximum MTU size

// Timeout Constants (milliseconds)
#define START_ACK_TIMEOUT_MS 10000    // Timeout waiting for START_ACK
#define BURST_ACK_TIMEOUT_MS 2000     // Timeout waiting for burst ACK
#define SEMAPHORE_POLL_MS 50          // Polling interval for semaphore
#define NOTIFY_RETRY_DELAY_MS 10      // Delay when notify buffer is full
#define BURST_PACKET_DELAY_MS 10      // Delay between packets in a burst

// Default Values
#define DEFAULT_CHUNK_BURST_SIZE 8    // Default number of chunks per burst
...
void transferFileChunked() {
  ...
  while (millis() - startAckWaitTime < START_ACK_TIMEOUT_MS) {
    if (digitalRead(REC_BUTTON_GPIO) == HIGH) {
    ...
    if (xSemaphoreTake(startTransferSemaphore, pdMS_TO_TICKS(SEMAPHORE_POLL_MS)) == pdTRUE) {
    ...
    if (file) {
      ...
      while (true) {
        ...
        for (int i = 0; i < chunk_burst_size_local; ++i) {
          bytesRead = file.read(buffer, BLE_PAYLOAD_SIZE);
          if (bytesRead <= 0) {
          ...
          memcpy(packet + BLE_HEADER_SIZE, buffer, bytesRead);

          pResponseCharacteristic->setValue(packet, bytesRead + BLE_HEADER_SIZE);
          while (!pResponseCharacteristic->notify()) {
            delay(NOTIFY_RETRY_DELAY_MS);
          }
          delay(BURST_PACKET_DELAY_MS);
          chunkCounter++;
        }
        ...
        while (millis() - ackWaitStartTime < BURST_ACK_TIMEOUT_MS) {
          if (digitalRead(REC_BUTTON_GPIO) == HIGH) {
            applog("Recording button pressed during ACK wait. Aborting.");
            transferAborted = true;
            break;
          }
          if (xSemaphoreTake(ackSemaphore, pdMS_TO_TICKS(SEMAPHORE_POLL_MS)) == pdTRUE) {
            ackReceived = true;
            break;
          }
        }
...
static void handle_get_file(const std::string& value) {
  ...
  if (last_colon_pos != std::string::npos) {
    ...
    if (g_chunk_burst_size <= 0) {
      g_chunk_burst_size = DEFAULT_CHUNK_BURST_SIZE;
    }
  } else {
