...
bool initI2SMicrophone() {
  ...
  if (i2s_driver_install(I2S_NUM_0, &i2s_config, 0, NULL) != ESP_OK) {
    applog("ERROR: Failed to install I2S driver!");
    return false;
  }
  if (i2s_set_pin(I2S_NUM_0, &pin_config) != ESP_OK) {
    applog("ERROR: Failed to set I2S pins!");
    i2s_driver_uninstall(I2S_NUM_0);  // Cleanup
    return false;
  }
  ...
  if (i2s_set_clk(I2S_NUM_0, I2S_SAMPLE_RATE, I2S_BITS_PER_SAMPLE_32BIT, I2S_CHANNEL_MONO) != ESP_OK) {  // Use I2S_CHANNEL_MONO as per existing code
    applog("ERROR: Failed to set I2S clock!");
    i2s_driver_uninstall(I2S_NUM_0);  // Cleanup
    return false;
  }
  ...
  if (g_buffer_mutex == NULL) {
    applog("ERROR: Failed to create buffer mutex!");
  ...
  if (USE_ADPCM) {
    ...
    if (g_adpcm_buffer_mutex == NULL) {
      applog("ERROR: Failed to create ADPCM buffer mutex!");
      vTaskDelete(g_i2s_reader_task_handle);
      vSemaphoreDelete(g_buffer_mutex);
      i2s_driver_uninstall(I2S_NUM_0);
      return false;
    }
...
void startRecording() {
  ...
  if (USE_ADPCM) {
    ...
    if (encoderCreated != pdPASS) {
      ...
      g_audioFile.close();
      LittleFS.remove(g_audio_filename);
      setAppState(IDLE);
      onboard_led(false);
      return;
    }
    ...
    if (writerCreated != pdPASS) {
      ...
      g_is_buffering = false;
      vTaskDelete(g_audio_writer_task_handle);  // Cleanup encoder task
